...
export function useChatWithPerplexity() {
  ...
  // 👇 команда для вывода MCP‑тулзов
  if (input.trim() === '/tools') {
    setIsLoading(true);
    try {
      const resp = await fetch('http://localhost:4000/api/mcp/tools');
      const data = await resp.json();

     const listResult = data.tools || {};
const toolsArray = listResult.tools || [];

const toolsList = toolsArray
  ...
  .join('\n');
      ...
      setMessages(prev => [
        ...
        { role: 'user', content: input },
        {
          ...
          content: toolsList
            ? `Доступные MCP‑тулзы:\n${toolsList}`
            : 'Тулзы MCP не найдены или сервер недоступен.',
        },
      ]);
    } catch (err) {
      setMessages(prev => [
        ...
        {
          role: 'assistant',
          content: `Ошибка при получении MCP‑тулзов: ${err.message}`,
        },
      ]);
    } finally {
      setIsLoading(false);
    }
    return; // важно завершить здесь, чтобы не улетело в /api/chat
  }
