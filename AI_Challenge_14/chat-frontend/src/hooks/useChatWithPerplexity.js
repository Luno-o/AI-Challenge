...
export function useChatWithPerplexity() {
  ...
  // 🆕 ДОБАВЛЕНО: Оркестратор для цепочки
  ...
  const determineToolsToCall = (userMessage) => {
    ...
    // 🆕 ДОБАВЛЕНО: Проверка на команду оркестратора
    if (lower.includes('суммаризируй') || lower.includes('отправь на github') || 
        lower.includes('push') || lower.includes('github') && lower.includes('summary')) {
      return ['orchestrate']; // Специальный флаг для оркестратора
    }
  ...
  const processAgentResponse = async (userMessage, systemPrompt) => {
    ...
    const enhancedSystemPrompt = `${systemPrompt}\n\n📚 Доступные Task tools:\n${toolsDescription}\n\n🎭 Специальная команда: Если пользователь просит "суммаризировать" или "отправить на GitHub", используй endpoint POST /api/orchestrate/summary-chain для выполнения полной цепочки.\n\nИспользуй полученные данные для ответа пользователю.`;
  ...
  const sendMessage = useCallback(async (userMessage) => {
    ...
    try {
      ...
      const systemPrompt = `Ты — Task Manager Agent с поддержкой GitHub интеграции.
Специализируешься на управлении задачами и автоматической синхронизации с GitHub.
Всегда давай четкие, структурированные ответы с emoji и логами выполненных операций.
Когда пользователь просит суммаризировать и отправить на GitHub — выполняй полную цепочку через оркестратор.`;
